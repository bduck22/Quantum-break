...
namespace BzKovSoft.ObjectSlicer
{
    ...
    public abstract class BzSliceableBase : MonoBehaviour, IBzMeshSlicer
    {
        ...
        [Header("Optimization")]
        public bool fastSimpleGrouping;
        ...
        {
            if (fastSimpleGrouping && !objectGrouping)
            {
                Profiler.BeginSample("GroupObjectsFast");
                OneObjectItem[] fastResult = GroupObjectsFast(itemMeshes);
                Profiler.EndSample();
        ...
        private static OneObjectItem[] GroupObjectsFast(List<OneObjectItemMesh> itemMeshes)
        {
            if (itemMeshes == null || itemMeshes.Count == 0)
            {
                return new OneObjectItem[0];
            }

            OneObjectItem negativeObject = null;
            OneObjectItem positiveObject = null;

            int counter = 0;
            int capacity = itemMeshes.Count / 2 + 1;

            for (int i = 0; i < itemMeshes.Count; i++)
            {
                OneObjectItemMesh itemMesh = itemMeshes[i];
                ...
                if (itemMesh.MeshData == null)
                {
                    continue;
                }

                if (itemMesh.MeshRenderer == null)
                {
                    continue;
                }

                OneObjectItem targetObject;

                if (itemMesh.Side)
                {
                    if (positiveObject == null)
                    {
                        positiveObject = new OneObjectItem
                        {
                            groupNumber = ++counter,
                            side = true,
                            meshes = new List<OneObjectItemMesh>(capacity),
                        };
                    }

                    targetObject = positiveObject;
                }
                else
                {
                    if (negativeObject == null)
                    {
                        negativeObject = new OneObjectItem
                        {
                            groupNumber = ++counter,
                            side = false,
                            meshes = new List<OneObjectItemMesh>(capacity),
                    ...
                    targetObject = negativeObject;
                }

                AddOrMergeFastMesh(targetObject, itemMesh);
            }

            if (negativeObject != null && positiveObject != null)
            {
                return new[]
                {
            negativeObject,
        ...
        private static void AddOrMergeFastMesh(OneObjectItem targetObject, OneObjectItemMesh itemMesh)
        {
            for (int i = 0; i < targetObject.meshes.Count; i++)
            {
                OneObjectItemMesh existingMesh = targetObject.meshes[i];

                if (existingMesh.MeshData == itemMesh.MeshData)
                {
                    existingMesh.AddMesh(itemMesh);
                    return;
                }
            }

            targetObject.meshes.Add(itemMesh);
        }
        ...
        private readonly struct BzTriangleBound
        {
            ...
            {
                minX = tr1.v1.x < tr1.v2.x ? tr1.v1.x : tr1.v2.x;
                minX = tr1.v3.x < minX ? tr1.v3.x : minX;

                maxX = tr1.v1.x > tr1.v2.x ? tr1.v1.x : tr1.v2.x;
                maxX = tr1.v3.x > maxX ? tr1.v3.x : maxX;

                minY = tr1.v1.y < tr1.v2.y ? tr1.v1.y : tr1.v2.y;
                minY = tr1.v3.y < minY ? tr1.v3.y : minY;

                maxY = tr1.v1.y > tr1.v2.y ? tr1.v1.y : tr1.v2.y;
                maxY = tr1.v3.y > maxY ? tr1.v3.y : maxY;

                minZ = tr1.v1.z < tr1.v2.z ? tr1.v1.z : tr1.v2.z;
                minZ = tr1.v3.z < minZ ? tr1.v3.z : minZ;

                maxZ = tr1.v1.z > tr1.v2.z ? tr1.v1.z : tr1.v2.z;
                maxZ = tr1.v3.z > maxZ ? tr1.v3.z : maxZ;
            }
        }
